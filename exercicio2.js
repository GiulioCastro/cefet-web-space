...
const imgSlideEl = document.querySelector('#slide');

imgSlideEl.src = `${servidorDasImagens}/${imagens[indiceImg].arquivo}`;
imgSlideEl.alt  = imagens[indiceImg].descricao;

const botaoAnteriorEl = document.querySelector('#anterior');
const botaoProximoEl = document.querySelector('#proximo');

botaoAnteriorEl.addEventListener('click', function () {
  indiceImg = indiceImg === 0 ? imagens.length - 1 : indiceImg - 1;
  imgSlideEl.src = `${servidorDasImagens}/${imagens[indiceImg].arquivo}`;
  imgSlideEl.alt  = imagens[indiceImg].descricao;
});

botaoProximoEl.addEventListener('click', function () {
  indiceImg = indiceImg === imagens.length - 1 ? 0 : indiceImg + 1;
  imgSlideEl.src = `${servidorDasImagens}/${imagens[indiceImg].arquivo}`;
  imgSlideEl.alt  = imagens[indiceImg].descricao;
});